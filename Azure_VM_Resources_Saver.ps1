...
###################################
Azure PowerShell (AzureRM) >= 0.7.0
###################################
...
$DeleteVMroutine = {
    ...
    try {
        ...
        if ([string]::IsNullOrEmpty($(Get-AzureRmContext).Account)) {Connect-AzureRmAccount -Message "Your user login session has expired, You need to login again as (current user)."}
        $VMName = $VM.Name
        ...
        if ($VM.DiagnosticsProfile.bootDiagnostics) {
            Write-Host -BackgroundColor Green "VM ($VMName) Step-1: Deleting boot diagnostics storage container."
        ...
        # Delete VM
        Write-Host -BackgroundColor Green "VM ($VMName) Step-2: Deleting the VM."
        Remove-AzureRmVM -Name $VMName -ResourceGroupName $VM.ResourceGroupName -Force
        # Delete NICs
        Write-Host -BackgroundColor Green "VM ($VMName) Step-3: Deleting NICs & PublicIPs."
        foreach ($nicUri in $VM.NetworkProfile.NetworkInterfaces) {
            ...
            Write-Host -BackgroundColor Green "VM ($VMName): Deleting NIC ($($nic.name)."
            ...
            foreach ($ipConfig in $nic.IpConfigurations) {
                if ($ipConfig.PublicIpAddress -ne $null) {
                    ...
                    Write-Host -BackgroundColor Green "VM ($VMName): Deleting PublicIP Address ($IPName)."
                    Remove-AzureRmPublicIpAddress -ResourceGroupName $VM.ResourceGroupName -Name $IPName -Force
                }
            }
        }
        # Delete disks
        Write-Host -BackgroundColor Green "VM ($VMName) Step-4: Deleting disks." 
        ...
        if ($VM.StorageProfile.OsDisk.ManagedDisk ) {
            ($OSDisk + $DataDisks) | ForEach-Object {
                Write-Host -BackgroundColor Green "VM ($VMName): Deleting managed disk: $_" 
                Get-AzureRmDisk -ResourceGroupName $VM.ResourceGroupName -DiskName $_ | Remove-AzureRmDisk -Force 
            }
        }
        else {
            # Delete OS disk
            Write-Host -BackgroundColor Green "VM ($VMName): Deleting unmanaged OS disk."
            ...
            # Delete status blob
            Write-Host -BackgroundColor Green "VM ($VMName): Deleting OS disk status blob."
            ...
            if ($DataDisks.Count -gt 0) {
                Write-Host -BackgroundColor Green "VM ($VMName): Deleting unmanaged data disks."
        ...
        Write-Host -BackgroundColor Green "Complete deletion of VM ($VMName) succeeded."
    }
    Catch {
...
try {
    ...
    if ([string]::IsNullOrEmpty($(Get-AzureRmContext).Account)) {Connect-AzureRmAccount}
    ...
    if ($VMs) {
        ...
        foreach ($VM in $VMs) {
            ...
            $DeleteVM_Job = Start-Job -ScriptBlock $DeleteVMroutine -ArgumentList $VM
        ...
        $End = Get-Date
        $scriptTime = New-TimeSpan -Start $Start -End $End
        $scriptTimeSeconds = $scriptTime.TotalSeconds
        Write-Host "This Script took $scriptTimeSeconds seconds to run"
    }
    else {
        Write-Host -BackgroundColor Green "There are no untagged VMs to delete in the provided subscription ($subscriptionID)."
    }
}
Catch {
    $ErrorMEssage = $_.Exception.Message
